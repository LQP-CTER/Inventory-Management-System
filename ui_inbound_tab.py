...
class InboundFrame(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.header_label = ctk.CTkLabel(self, text="Inbound Stock", font=("Inter", 24, "bold"))
        ...
        # Unit Price
        ctk.CTkLabel(self.form_frame, text="Unit Price:", font=("Inter", 14)).grid(row=1, column=2, padx=15, pady=15, sticky="w")
        self.price_entry = ctk.CTkEntry(self.form_frame, width=200)
        self.price_entry.grid(row=1, column=3, padx=15, pady=15)
        ...
        # Submit Button
        self.btn_submit = ctk.CTkButton(self, text="Confirm Inbound", command=self.submit_inbound, 
                                        font=("Inter", 16, "bold"), fg_color="#10b981", height=45)
    ...
    def add_to_list(self):
        ...
        if not price.replace('.','',1).isdigit():
            show_toast(self.controller, "Price must be a valid number.", type="warning")
            return
    ...
    def update_list_view(self):
        ...
        for i, item in enumerate(self.added_items):
            ...
            text = f"📦 {item['product']} | 🏭 {item['warehouse']} | Qty: {item['qty']} | 💵 ${item['price']}"
    ...
    def submit_inbound(self):
        if not self.added_items:
            show_toast(self.controller, "No items to inbound!", type="warning")
            return
        ...
        try:
            ...
            for item in self.added_items:
                ...
                trans = InventoryTransaction(
                    Transaction_ID=trans_id, Transaction_Type='Inbound', 
                ...
                detail = InventoryTransactionDetail(
                    ...
                    Product_ID=prod_id, Quantity=item["qty"], Total_Price=item["price"] * item["qty"],
                    Unit_Price=item["price"]
                )
                self.session.add(detail)
                
                # NOTE: We DO NOT update InventoryBalance here because status is Draft.
                # It will be updated by Admin in Transactions tab upon approval.

                audit = AuditLog(Username=username, Action=f"Created Inbound Draft: {item['product']} (Qty: {item['qty']})", Table_Affected="inventory_transaction", Timestamp=trans_date)
                self.session.add(audit)

            self.session.commit()
            show_toast(self.controller, "Inbound draft recorded successfully!", type="success")
